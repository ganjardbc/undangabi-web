...
export default {
  ...
  methods: {
    ...
    async removeData() {
      ...
      const id = this.onSearchData(this.selectedIndex).theme_id
      const payload = {
        theme_id: id,
      }

      const rest = await axios.post('/api/theme/delete', payload, {
    ...
    async saveData() {
      ...
      const url =
        this.formTitle === 'CREATE' ? '/api/theme/post' : '/api/theme/update'
    ...
    async uploadImage(data) {
      ...
      const url = '/api/theme/uploadImage'
    ...
    async removeImage() {
      ...
      const token = 'Bearer '.concat(this.$cookies.get('token'))
      const payload = this.selectedData
      const url = '/api/theme/removeImage'
    ...
    async getData() {
      ...
      const rest = await axios.post('/api/theme/getAll', payload, {
        headers: { Authorization: token },
      })
