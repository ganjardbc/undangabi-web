    ...
    <div class="left">
      <div class="bg-white box-shadow">
        <div
          ...
          style="height: 40px"
        >
          <h1 class="fonts small black">GUESTS</h1>
          ...
          >
            ...
            <el-table-column label="Present">
              <template slot-scope="scope">
                <div class="display-flex">
                  <div
                    v-if="scope.row.guest.present_type === 'present'"
                  ...
                  <div
                    v-if="scope.row.guest.present_type === 'not-present'"
                    :class="'card-capsule'"
                    style="text-transform: capitalize; line-height: 0.6"
                  >
                    Not Present
                  ...
                  <div
                    v-if="scope.row.guest.present_type === 'will-present'"
                  ...
                  <div
                    v-if="scope.row.guest.present_type === 'waiting-response'"
                    :class="'card-capsule wip'"
                    style="text-transform: capitalize; line-height: 0.6"
                  >
                    Waiting Response
            ...
            <el-table-column label="Status" width="150">
              <template slot-scope="scope">
                <div class="display-flex">
                  <div
                    :class="`card-capsule ${scope.row.guest.status === 'active' && 'active'}`"
                    style="text-transform: capitalize; line-height: 0.6"
                  >
                    {{ scope.row.guest.status }}
                  </div>
                </div>
              </template>
            </el-table-column>
                ...
                <el-popover placement="bottom-end" width="180" trigger="click">
                  <div class="width width-100">
                    <button
                      ...
                      @click="onShow('VIEW', scope.row.guest.id)"
                    >
                      <i class="icn icn-left fa fa-lw fa-align-left"></i> Detail
                    </button>
                    <button
                      class="btn btn-white btn-full btn-align-left"
                      @click="onShow('EDIT', scope.row.guest.id)"
                    >
                      <i class="icn icn-left fa fa-lw fa-edit"></i> Edit
                    </button>
                    <button
                      class="btn btn-white btn-full btn-align-left"
                      @click="onShowHideDelete(scope.row.guest.id)"
                    >
...
export default {
  ...
  methods: {
    ...
    async removeData() {
      ...
      const id = this.onSearchData(this.selectedIndex).guest.guest_id
      const payload = {
        guest_id: id,
      }

      const rest = await axios.post('/api/guest/delete', payload, {
    ...
    async saveData() {
      ...
      const url =
        this.formTitle === 'CREATE' ? '/api/guest/post' : '/api/guest/update'
    ...
    async uploadImage(data) {
      ...
      const token = 'Bearer '.concat(this.$cookies.get('token'))
      const payload = this.selectedData.guest
      const url = '/api/guest/uploadImage'

      let formData = new FormData()
      formData.append('guest_id', payload.guest_id)
    ...
    async removeImage() {
      ...
      const token = 'Bearer '.concat(this.$cookies.get('token'))
      const payload = this.selectedData.guest
      const url = '/api/guest/removeImage'
    ...
    async getData() {
      ...
      const rest = await axios.post('/api/guest/getAll', payload, {
        headers: { Authorization: token },
      })
