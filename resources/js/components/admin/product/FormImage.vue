      ...
      <div class="display-flex">
        <div class="width width-60-px">
          <div class="image image-45px">
            <img
              :src="'/contents/products/thumbnails/' + dt.image"
              alt="SAJI-IN"
            />
          </div>
        </div>
    ...
    >
      ...
      <div class="field-group margin margin-bottom-15-px">
        ...
        >
          {{
            formMessage &&
            formMessage.prodimage_id &&
            formMessage.prodimage_id[0]
          }}
        </div>
      </div>
      <div
        v-if="isCreate ? true : false"
...
export default {
  ...
  methods: {
    ...
    async removeData() {
      ...
      const rest = await axios.post('/api/productImage/delete', payload, {
    ...
    async saveData() {
      ...
      const url = this.isCreate
        ? '/api/productImage/post'
        : '/api/productImage/update'
    ...
    async getData(id) {
      ...
      const rest = await axios.post('/api/productImage/getAll', payload, {
        headers: { Authorization: token },
      })
