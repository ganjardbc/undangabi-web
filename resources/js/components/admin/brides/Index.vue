    ...
    <div class="left">
      <div class="bg-white box-shadow">
        <div
          ...
          style="height: 40px"
        >
          <h1 class="fonts small black">BRIDES</h1>
          ...
          >
            ...
            <el-table-column label="Status" width="150">
              <template slot-scope="scope">
                <div class="display-flex">
                  <div
                    :class="`card-capsule ${scope.row.bride.status === 'active' && 'active'}`"
                    style="text-transform: capitalize; line-height: 0.6"
                  >
                    {{ scope.row.bride.status }}
                  </div>
                </div>
              </template>
            </el-table-column>
                ...
                <el-popover placement="bottom-end" width="180" trigger="click">
                  <div class="width width-100">
                    <button
                      ...
                      @click="onShow('VIEW', scope.row.bride.id)"
                    >
                      <i class="icn icn-left fa fa-lw fa-align-left"></i> Detail
                    </button>
                    <button
                      class="btn btn-white btn-full btn-align-left"
                      @click="onShow('EDIT', scope.row.bride.id)"
                    >
                      <i class="icn icn-left fa fa-lw fa-edit"></i> Edit
                    </button>
                    <button
                      class="btn btn-white btn-full btn-align-left"
                      @click="onShowHideDelete(scope.row.bride.id)"
                    >
...
export default {
  ...
  methods: {
    ...
    async removeData() {
      ...
      const id = this.onSearchData(this.selectedIndex).bride.bride_id
      const payload = {
        bride_id: id,
      }

      const rest = await axios.post('/api/bride/delete', payload, {
    ...
    async saveData() {
      ...
      const url =
        this.formTitle === 'CREATE' ? '/api/bride/post' : '/api/bride/update'
    ...
    async uploadImage(data) {
      ...
      const token = 'Bearer '.concat(this.$cookies.get('token'))
      const payload = this.selectedData.bride
      const url = '/api/bride/uploadImage'
    ...
    async removeImage() {
      ...
      const token = 'Bearer '.concat(this.$cookies.get('token'))
      const payload = this.selectedData.bride
      const url = '/api/bride/removeImage'
