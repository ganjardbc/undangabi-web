    ...
    <div class="left">
      <div class="bg-white box-shadow">
        <div
          ...
          style="height: 40px"
        >
          <h1 class="fonts small black">GALLERIES</h1>
          ...
          >
            ...
            <el-table-column label="Description">
              <template slot-scope="scope">
                <div>{{ scope.row.gallery.description }}</div>
              </template>
            </el-table-column>
            <el-table-column label="Type">
              <template slot-scope="scope">
                <div>{{ scope.row.gallery.type }}</div>
              </template>
            </el-table-column>
            ...
            <el-table-column label="Status" width="150">
              <template slot-scope="scope">
                <div class="display-flex">
                  <div
                    :class="`card-capsule ${scope.row.gallery.status === 'active' && 'active'}`"
                    style="text-transform: capitalize; line-height: 0.6"
                  >
                    {{ scope.row.gallery.status }}
                  </div>
                </div>
              </template>
            </el-table-column>
                ...
                <el-popover placement="bottom-end" width="180" trigger="click">
                  <div class="width width-100">
                    <button
                      ...
                      @click="onShow('VIEW', scope.row.gallery.id)"
                    >
                      <i class="icn icn-left fa fa-lw fa-align-left"></i> Detail
                    </button>
                    <button
                      class="btn btn-white btn-full btn-align-left"
                      @click="onShow('EDIT', scope.row.gallery.id)"
                    >
                      <i class="icn icn-left fa fa-lw fa-edit"></i> Edit
                    </button>
                    <button
                      class="btn btn-white btn-full btn-align-left"
                      @click="onShowHideDelete(scope.row.gallery.id)"
                    >
...
export default {
  ...
  methods: {
    ...
    async removeData() {
      ...
      const id = this.onSearchData(this.selectedIndex).gallery.gallery_id
      const payload = {
        gallery_id: id,
      }

      const rest = await axios.post('/api/gallery/delete', payload, {
    ...
    async uploadImage(data) {
      ...
      const token = 'Bearer '.concat(this.$cookies.get('token'))
      const payload = this.selectedData.gallery
      const url = '/api/gallery/uploadImage'

      let formData = new FormData()
      formData.append('gallery_id', payload.gallery_id)
    ...
    async removeImage() {
      ...
      const token = 'Bearer '.concat(this.$cookies.get('token'))
      const payload = this.selectedData.gallery
      const url = '/api/gallery/removeImage'
    ...
    async getData() {
      ...
      const rest = await axios.post('/api/gallery/getAll', payload, {
        headers: { Authorization: token },
      })
