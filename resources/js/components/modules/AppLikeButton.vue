<template>
  <button
    id="AppLikeButton"
    :class="
      isBigButton ? 'btn btn-icon btn-white' : 'btn btn-small-icon btn-white'
    "
    @click="status ? setStatusWishelist('delete') : setStatusWishelist('save')"
  >
    <i v-if="visibleLoader" class="fa fa-lg fa-spinner fa-spin"></i>
    <i
      v-else
      :class="status ? 'fa fa-lg fa-heart' : 'far fa-lg fa-heart'"
      :style="'font-size: 18px; ' + (status && 'color: red;')"
    ></i>
  </button>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    async deleteStatus(data) {
      ...
      return await axios
        .post('/api/wishelist/delete', payload, {
    ...
    async postStatus(data) {
      ...
      return await axios
        .post('/api/wishelist/post', payload, {
          headers: { Authorization: data.token },
        })
        .then((rest) => {
          if (rest && rest.status === 200) {
    ...
    async getStatus(data) {
      ...
      return await axios
        .post('/api/wishelist/checkWisheList', payload, {
          headers: { Authorization: data.token },
        })
        .then((rest) => {
          if (rest && rest.status === 200) {
