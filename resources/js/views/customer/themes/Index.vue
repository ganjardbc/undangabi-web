<template>
  <div id="App">
    <AppBreadCrumb title="Daftar Tema" />
    ...
    <div class="invitation-main-footer">
      ...
      >
        <button class="btn btn-main btn-full" @click="submitData">
          <i class="icn icn-left fa fa-lg fa-plus-circle"></i> Simpan Tema
        </button>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      visibleAlert: false,
      visibleLoader: false,
      titleConfirmed: 'Simpan tema ?',
      titleAlert: 'Tema berhasil di simpan !',
  ...
  methods: {
    ...
    submitData() {
      ...
      this.titleConfirmed = 'Simpan tema ?'
    },
    onCloseUpdateCover() {
      ...
      this.invitationUpdateData({ token })
        .then((res) => {
          if (res.data.status === 'ok') {
            this.$message('Tema berhasil di simpan !')
          } else {
            this.visibleAlert = true
            this.titleAlert = 'Tema gagal di simpan !'
