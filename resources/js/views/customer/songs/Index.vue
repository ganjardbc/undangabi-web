<template>
  <div id="App">
    <AppBreadCrumb title="Lagu Latar" />
    ...
    <div class="invitation-main-footer">
      ...
      >
        <button class="btn btn-main btn-full" @click="submitData">
          <i class="icn icn-left fa fa-lg fa-plus-circle"></i> Simpan Lagu Latar
        </button>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      visibleAlert: false,
      visibleLoader: false,
      titleConfirmed: 'Simpan data lagu latar ?',
      titleAlert: 'Lagu latar berhasil di simpan !',
  ...
  methods: {
    ...
    submitData() {
      ...
      this.titleConfirmed = 'Simpan lagu latar ?'
    },
    onCloseUpdateCover() {
      ...
      this.invitationUpdateData({ token })
        .then((res) => {
          if (res.data.status === 'ok') {
            this.$message('Lagu latar berhasil di simpan !')
          } else {
            this.visibleAlert = true
            this.titleAlert = 'Lagu latar gagal di simpan !'
