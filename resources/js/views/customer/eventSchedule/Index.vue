<template>
  <div id="App">
    <AppBreadCrumb title="Jadwal Acara" />
...
export default {
  ...
  data() {
    return {
      ...
      visibleAlert: false,
      visibleLoader: false,
      titleConfirmed: 'Simpan Jadwal Acara ?',
      titleAlert: 'Jadwal Acara berhasil di simpan !',
  ...
  methods: {
    ...
    submitData() {
      ...
      this.titleConfirmed = 'Simpan Jadwal Acara ?'
    },
    onCloseUpdateCover() {
      ...
      this.invitationUpdateData({ token })
        .then((res) => {
          if (res.data.status === 'ok') {
            this.$message('Jadwal Acara berhasil di simpan !')
          } else {
            this.visibleAlert = true
            this.titleAlert = 'Jadwal Acara gagal di simpan !'
