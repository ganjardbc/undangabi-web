    ...
    <div class="left">
      <div class="bg-white box-shadow">
        <div
          ...
          style="height: 40px"
        >
          <h1 class="fonts small black">SPECIAL THANKS</h1>
          <div class="display-flex">
            ...
            <SearchField
              :placeholder="'Search special thanks ..'"
          ...
          >
            ...
            <el-table-column label="Name">
              <template slot-scope="scope">
                <div>{{ scope.row.specialthanks.name }}</div>
              </template>
            </el-table-column>
            ...
            <el-table-column label="Status" width="150">
              <template slot-scope="scope">
                <div class="display-flex">
                  <div
                    :class="`card-capsule ${scope.row.specialthanks.status === 'active' && 'active'}`"
                    style="text-transform: capitalize; line-height: 0.6"
                  >
                    {{ scope.row.specialthanks.status }}
                  </div>
                </div>
              </template>
            </el-table-column>
                ...
                <el-popover placement="bottom-end" width="180" trigger="click">
                  <div class="width width-100">
                    <button
                      ...
                      @click="onShow('VIEW', scope.row.specialthanks.id)"
                    >
                      <i class="icn icn-left fa fa-lw fa-align-left"></i> Detail
                    </button>
                    <button
                      class="btn btn-white btn-full btn-align-left"
                      @click="onShow('EDIT', scope.row.specialthanks.id)"
                    >
                      <i class="icn icn-left fa fa-lw fa-edit"></i> Edit
                    </button>
                    <button
                      class="btn btn-white btn-full btn-align-left"
                      @click="onShowHideDelete(scope.row.specialthanks.id)"
                    >
  ...
  methods: {
    ...
    onChangeImage(data) {
      this.selectedData = {
        ...this.selectedData,
        specialthanks: {
          ...this.selectedData.specialthanks,
          image: data,
        },
      }
    },
    async removeData() {
      ...
      const id = this.onSearchData(this.selectedIndex).specialthanks
        .specialthanks_id
      const payload = {
        specialthanks_id: id,
      }

      const rest = await axios.post('/api/specialthanks/delete', payload, {
    ...
    async saveData() {
      ...
      const url =
        this.formTitle === 'CREATE'
          ? '/api/specialthanks/post'
          : '/api/specialthanks/update'
    ...
    async uploadImage(data) {
      ...
      const token = 'Bearer '.concat(this.$cookies.get('token'))
      const payload = this.selectedData.specialthanks
      const url = '/api/specialthanks/uploadImage'

      let formData = new FormData()
      formData.append('specialthanks_id', payload.specialthanks_id)
    ...
    async removeImage() {
      ...
      const token = 'Bearer '.concat(this.$cookies.get('token'))
      const payload = this.selectedData.specialthanks
      const url = '/api/specialthanks/removeImage'
    ...
    async getData() {
      ...
      const rest = await axios.post('/api/specialthanks/getAll', payload, {
        headers: { Authorization: token },
      })
