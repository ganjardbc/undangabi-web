    ...
    <div class="left">
      <div class="bg-white box-shadow">
        <div
          ...
          style="height: 40px"
        >
          <h1 class="fonts small black">EVENTS</h1>
          ...
          >
            ...
            <el-table-column label="Status" width="150">
              <template slot-scope="scope">
                <div class="display-flex">
                  <div
                    :class="`card-capsule ${scope.row.event.status === 'active' && 'active'}`"
                    style="text-transform: capitalize; line-height: 0.6"
                  >
                    {{ scope.row.event.status }}
                  </div>
                </div>
              </template>
            </el-table-column>
                ...
                <el-popover placement="bottom-end" width="180" trigger="click">
                  <div class="width width-100">
                    <button
                      ...
                      @click="onShow('VIEW', scope.row.event.id)"
                    >
                      <i class="icn icn-left fa fa-lw fa-align-left"></i> Detail
                    </button>
                    <button
                      class="btn btn-white btn-full btn-align-left"
                      @click="onShow('EDIT', scope.row.event.id)"
                    >
                      <i class="icn icn-left fa fa-lw fa-edit"></i> Edit
                    </button>
                    <button
                      class="btn btn-white btn-full btn-align-left"
                      @click="onShowHideDelete(scope.row.event.id)"
                    >
...
export default {
  ...
  methods: {
    ...
    async removeData() {
      ...
      const id = this.onSearchData(this.selectedIndex).event.event_id
      const payload = {
        event_id: id,
      }

      const rest = await axios.post('/api/event/delete', payload, {
    ...
    async saveData() {
      ...
      const url =
        this.formTitle === 'CREATE' ? '/api/event/post' : '/api/event/update'
    ...
    async uploadImage(data) {
      ...
      const token = 'Bearer '.concat(this.$cookies.get('token'))
      const payload = this.selectedData.event
      const url = '/api/event/uploadImage'
    ...
    async removeImage() {
      ...
      const token = 'Bearer '.concat(this.$cookies.get('token'))
      const payload = this.selectedData.event
      const url = '/api/event/removeImage'
    ...
    async getData() {
      ...
      const rest = await axios.post('/api/event/getAll', payload, {
        headers: { Authorization: token },
      })
