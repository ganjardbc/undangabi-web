<template>
  <div id="WebLayouts">
    <div v-if="invitation" id="header" class="box-shadow">
      <div id="header-container" style="align-items: center">
        <div class="hc-logo">
          <router-link
            :to="{ name: 'generate-invitation-private' }"
        ...
        <button
          ...
          @click="onOpenPopup"
          :disabled="invitation && invitation.privacy === 'public'"
        >
    ...
    <div v-loading="visibleLoader" id="body">
      <div
        v-if="invitation && invitation.privacy === 'public'"
        class="small-screen"
      >
        <AppEmpty title="Fitur Ini Untuk Undangan Private" />
      </div>
      <div v-else id="themes-guest-lists" class="small-screen">
        ...
        <div class="padding padding-left-10px padding-right-10px">
          ...
          <div v-for="(dt, i) in filteredData" :key="i" style="margin: 15px 0">
            ...
            >
              <div class="width width-100 padding padding-bottom-15px">
                ...
                >
                  ...
                  <div class="display-flex flex-end align-center">
                    ...
                    <!-- <button 
                                            class="btn btn-white btn-icon margin margin-left-5px" 
                                            @click="onShowAlertDelete(dt)">
                                            <i class="icn fa fa-lg fa-trash-alt"></i>
                                        </button> -->
                  </div>
                </div>
                <div class="width width-100">
                  ...
                  <div
                    v-if="dt.comments"
                    class="fonts fonts-10 normal grey overflow-ellipsis"
                  >
                    {{ dt.comments }}
                  </div>
                  <div
              ...
              <div v-if="invitation" class="width width-100">
                <AppCardCollapse title="Lihat Kalimat Undangan">
                  <AppTextInvitation
                    :brides="generateInvitationTitle(invitation, dt.brides)"
                    :title="invitation.title"
                    :tags="invitation.tag"
                    :categoryId="invitation.category_id"
                    :shortLink="invitation.short_link"
                    :date="invitation.date"
                    :location="invitation.description"
                    :guestId="dt.reservation_list_id"
                    :guestName="dt.name"
                    :guestAddress="dt.address"
                    :textId="`target-invitation-${dt.reservation_list_id}`"
              ...
              <div class="display-flex space-between padding padding-top-15px">
                <button
                  ...
                  @click="
                    onCopyInvitation(
                      `target-invitation-${dt.reservation_list_id}`,
                      dt.name
                    )
                  "
                >
...
export default {
  ...
  methods: {
    ...
    async saveData() {
      ...
      const custom_reservation_list_id = `GE-${time}`
    ...
    async deleteData() {
      ...
      const url = '/api/public/reservationDelete'
