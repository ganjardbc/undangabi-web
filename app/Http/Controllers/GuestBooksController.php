...
class GuestBooksController extends Controller
{
    ...
    public function getAll(Request $req)
    {
        ...
        else 
        {
            ...
            if ($uID) 
            {
                $data = GuestBook::where($newStatus)
                    ...
                    ->orderBy('id', 'desc')
                    ->get();
                $totalRecord = GuestBook::where($newStatus)    
                    ->where('name', 'LIKE', '%'.$req['search'].'%')
                    ->where($isPresent)
                    ->where(['created_by' => $uID])
                    ->count();
    ...
    public function getQuantity(Request $req)
    {
        ...
        else 
        {
            ...
            if ($uID) 
            {
                $totalPresent = GuestBook::where($status)    
                    ->where(['present_type' => 'present'])
                    ->where(['created_by' => $uID])
                    ->sum('attendance');
                $totalWillPresent = GuestBook::where($status)    
                    ->where(['present_type' => 'will-present'])
                    ->where(['created_by' => $uID])
                    ->sum('attendance');
                $totalNotPresent = GuestBook::where($status)    
                    ->where(['present_type' => 'not-present'])
                    ->where(['created_by' => $uID])
                    ->count();
                $totalWaitingResponse = GuestBook::where($status)    
    ...
    {
        $validator = Validator::make($req->all(), [
            'guest_book_id' => 'required|string|min:0',
        ]);
        ...
        {
            $guest_book_id = $req['guest_book_id'];
            $data = GuestBook::where(['guest_book_id' => $guest_book_id])->first();
    ...
    public function removeImage(Request $req) 
    {
        ...
        else 
        {
            ...
            $filename = GuestBook::where(['guest_book_id' => $req['guest_book_id']])->first()->image;
            $data = GuestBook::where(['guest_book_id' => $req['guest_book_id']])->update($payload);

            if ($data)
            {
                unlink(public_path('contents/guestbooks/thumbnails/'.$filename));
				unlink(public_path('contents/guestbooks/covers/'.$filename));

                $response = [
                    'message' => 'proceed success',
                    'status' => 'ok',
                    'code' => '201',
                    'data' => GuestBook::where(['guest_book_id' => $req['guest_book_id']])->first()
                ];
            }
            else 
            {
    ...
    public function uploadImage(Request $req) 
    {
        ...
        else 
        {
			...
			$destination = public_path('contents/guestbooks/thumbnails/'.$filename);
			...
			$destination = public_path('contents/guestbooks/covers/');
    ...
    public function bulkDownload(Request $req)
    {
        ...
        else 
        {
            ...
            $data = GuestBook::where(['invitation_id' => $id])->get();
            ...
            for ($i=0; $i < count($raw_data); $i++) { 
                ...
                if ($i != 0) {
                    $importData_arr[$i] = $payload = [
                        'guest_book_id' => "GE-" . $id . "-" . rand(000000000000,999999999999),
                        'name' => $raw_data[$i][0],
                        'phone' => null,
    ...
    {
        $validator = Validator::make($req->all(), [
            'guest_book_id' => 'required|string|min:0|unique:guest_books',
    ...
    public function delete(Request $req)
    {
        ...
        {
            $data = GuestBook::where(['guest_book_id' => $req['guest_book_id']])->delete();
